...
public class ChatManager : MonoBehaviour
{
    ...
    [ReadOnly]
    public int currentIndex = 0;  // 当前显示的聊天信息的索引
    [ReadOnly]
    public bool isChat;  // 当前是否在进行聊天（标志位）
    [ReadOnly]
    public bool isSelected;  // 当前是否选择了按钮（标志位）
    [ReadOnly]
    public List<Button> currentSelectBtn = new List<Button>();  // 当前可用的选择按钮列表
    ...
    public void Update()
    {
        if (Input.GetMouseButtonDown(0))  // 判断是否按下了鼠标左键
        {
            ...
            // 如果当前聊天内容已显示完且正在进行聊天
            if (chatContent.text.Length >= chatInfos[currentIndex].chatContent.Length * ChatContentRot && isChat)
            {
            ...
            if (chatContent.text == chatInfos[currentIndex - 1].chatContent && chatInfos[currentIndex - 1].isStop)
            {
                gameObject.SetActive(false);  // 关闭聊天界面
                if (ischatEnd() == false)  // 判断是否为聊天结束状态
                {
                    chatButton.gameObject.SetActive(true);  // 显示聊天开始按钮
                    EventSystem.current.SetSelectedGameObject(chatButton.gameObject);  // 设置选中的按钮为聊天开始按钮
                }
                chatInfos[currentIndex - 1].chatEndEvent?.Invoke();  // 如果有聊天结束事件，则触发
                return;
            }
    ...
    public IEnumerator CreateChatBtns(int index)
    {
        if (chatInfos[index].selectsbtn.Count > 0)  // 如果当前聊天项有选择按钮
        {
            ...
            for (int i = 0; i < chatInfos[index].selectsbtn.Count; i++)  // 遍历所有选择按钮
            {
                int n = i;
                Button selectBtn = selectBtnPool.GetChild(0).GetComponent<Button>();  // 从按钮池获取一个按钮
                currentSelectBtn.Add(selectBtn);  // 将按钮添加到当前按钮列表
                selectBtn.transform.SetParent(selectBtnLayout);  // 设置按钮的父物体
                selectBtn.transform.GetChild(0).GetComponent<TMP_Text>().text = chatInfos[index].selectsbtn[i].btnContent;  // 设置按钮文本
                selectBtn.onClick.RemoveAllListeners();  // 移除按钮上的所有事件监听
                selectBtn.onClick.AddListener(() =>
                {
                    chatInfos[index].selectsbtn[n].btnEvent?.Invoke();  // 点击按钮时触发对应的事件
                    for (int j = 0; j < currentSelectBtn.Count; j++)  // 移除所有生成的按钮
                    {
                        currentSelectBtn[j].transform.SetParent(selectBtnPool);
                    }
                    ...
                    if (chatInfos[currentIndex - 1].isStop)  // 如果聊天结束
                    {
                        ...
                        {
                            chatButton.gameObject.SetActive(true);  // 显示聊天开始按钮
                            EventSystem.current.SetSelectedGameObject(chatButton.gameObject);  // 设置选中的按钮
                        }
                    }
                    else
                    {
