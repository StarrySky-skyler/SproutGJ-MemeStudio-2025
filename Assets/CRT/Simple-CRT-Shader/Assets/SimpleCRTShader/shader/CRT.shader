    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float flickeringMask = pow(
                    abs(sin(i.uv.y * _FlickeringCycle + _Time.y)), 10);
                uv.x = uv.x + (flickeringNoise * _FlickeringStrength *
                    flickeringMask);
                /////

                /////slippage
                float scrollSpeed = _Time.x * _SlippageScrollSpeed;
                float slippageMask = pow(
                    abs(sin(i.uv.y * _SlippageInterval + scrollSpeed)),
                    _SlippageSize);
                float stepMask = round(
                    sin(i.uv.y * _SlippageInterval + scrollSpeed - 1));
                uv.x = uv.x + (_SlippageNoiseOnOff * _SlippageStrength *
                    slippageMask * stepMask) * _SlippageOnOff;
                /////

                /////Chromatic Aberration
                float red = tex2D(_MainTex,
                    float2(
           ...
           _MultipleGhostOnOff);
                ...
                /////Letter box
                float band_uv = fmod(_MainTex_TexelSize.z, 640) /
                    _MainTex_TexelSize.z / 2;
        ...
        quality)
                ...
                {
                    return frac(
                        sin(dot(i.uv, float2(12.9898, 78.233)) + _Time.x) *
                        43758.5453);
                }
                /////
                ...
                (i.decaluv - _DecalTexPos) * _DecalTexScale) * _DecalTexOnOff;
                ...
                {
                    color.xyz = 0.299f * color.r + 0.587f * color.g + 0.114f *
                        color.b;
                }
                //////
