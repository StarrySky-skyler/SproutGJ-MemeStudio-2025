...
namespace Febucci.UI.Core
{
    ...
    public class EmissionCurvePropertyDrawer : PropertyDrawer
    {
            ...
            GUIContent label)
        {
            ...
            property.isExpanded = EditorGUI.Foldout(lineByLine,
            ...
            SerializedProperty cycles =
                property.FindPropertyRelative(nameof(EmissionCurve.cycles));
            Rect half = lineByLine;
            half.width /= 2f;
            EditorGUI.PropertyField(half, cycles);
            half.x += half.width + 5;
            half.width -= 5;
            EditorGUI.LabelField(half,
                cycles.intValue > 0
                    ? "cycles until end"
                    : "effect is infinite");
            ...
            EditorGUI.PropertyField(lineByLine,
                property.FindPropertyRelative(nameof(EmissionCurve.duration)));
            lineByLine.y += lineByLine.height;
            EditorGUI.PropertyField(lineByLine,
                property.FindPropertyRelative(
                    nameof(EmissionCurve.weightOverTime)));
        }
