...
namespace Febucci.UI.Core
{
    ...
    internal class TAnimCoreDrawer : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            typewriterStartsAutomatically =
                serializedObject.FindProperty("typewriterStartsAutomatically");

            useDefaultDatabases =
                serializedObject.FindProperty("useDefaultDatabases");
            databaseBehaviorsField =
                serializedObject.FindProperty("databaseBehaviors");
            databaseAppearancesField =
                serializedObject.FindProperty("databaseAppearances");
            databaseActionsField =
                serializedObject.FindProperty("databaseActions");
            ...
            defaultTagsMode =
                serializedObject.FindProperty(
                    nameof(TAnimCore.defaultTagsMode));
            defaultAppearancesTags =
                serializedObject.FindProperty("defaultAppearancesTags");
            defaultBehaviorsTags =
                serializedObject.FindProperty("defaultBehaviorsTags");
            defaultDisappearancesTags =
                serializedObject.FindProperty("defaultDisappearancesTags");
            ...
            useDynamicScaling =
                serializedObject.FindProperty(
                    nameof(TAnimCore.useDynamicScaling));
            timeScale =
                serializedObject.FindProperty(nameof(TAnimCore.timeScale));
            animationLoop =
                serializedObject.FindProperty(nameof(TAnimCore.animationLoop));
        ...
        private void DrawStyleSheet()
        {
            ...
            EditorGUILayout.LabelField(
                useDefaultStylesheet.boolValue
                    ...
                    : "Using the specific StyleSheet reference below",
            ...
            if (useDefaultStylesheet.boolValue)
            {
                ...
                {
                    GUI.enabled = false;
                    var defaultSettingsField =
                        settingsObject.FindProperty(
                            nameof(settings.defaultStyleSheet));
        ...
        private void DrawMainSettings()
        {
            ...
            if (typewriterStartsAutomatically.boolValue)
                EditorGUILayout.LabelField(
                    "(Remember to add a Typewriter component!)",
                    EditorStyles.wordWrappedMiniLabel);
        ...
        private void DrawHelpers()
        {
            ...
            if (GUILayout.Button("Select Global Settings",
            ...
            if (GUILayout.Button("Open Documentation", EditorStyles.helpBox))
                Application.OpenURL(
                    "https://www.febucci.com/text-animator-unity/docs/");
        ...
        private void DrawPlayback()
        {
            ...
            EditorGUILayout.LabelField(
                Application.isPlaying
                    ? "Playback info"
                    : "Preview in Edit Mode", GUILayout.Width(120));
            GUI.enabled = !Application.isPlaying;
            if (GUILayout.Button(
                    runInEditMode
                        ? TexturesLoader.StopIcon
                        : TexturesLoader.PlayIcon, EditorStyles.label,
            ...
            if (GUILayout.Button(TexturesLoader.SaveIcon, EditorStyles.label,
                    GUILayout.Width(20), GUILayout.Height(20)))
            {
            ...
            if (isPlaying)
            {
                ...
                if (!Application.isPlaying)
                {
                    ...
                    textDuringPreview =
                        EditorGUILayout.TextArea(textDuringPreview,
                ...
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.MinMaxSlider(gui_visibleCharacters,
                    ref minValue, ref maxValue, 0, charCount);
                if (EditorGUI.EndChangeCheck())
                {
                ...
                EditorGUI.BeginChangeCheck();
                intMinValue = EditorGUILayout.DelayedIntField(GUIContent.none,
                    intMinValue, GUILayout.Width(30));
                EditorGUILayout.LabelField("/", GUILayout.Width(10));
                intMaxValue = EditorGUILayout.DelayedIntField(GUIContent.none,
                    intMaxValue, GUILayout.Width(30));
                if (EditorGUI.EndChangeCheck())
                {
                    script.firstVisibleCharacter =
                        Mathf.Clamp(intMinValue, 0, charCount);
                    script.maxVisibleCharacters =
                        Mathf.Clamp(intMaxValue, 0, charCount);
                }
                ...
                if (extraPlaybackControls)
                {
                    ...
                    if (script.WordsCount > 20)
                        EditorGUILayout.HelpBox(
                            "Displaying only the first 20 words to optimize performance",
                            MessageType.None);

                    playbackCharsScrollView =
                        EditorGUILayout.BeginScrollView(
                            playbackCharsScrollView);

                    EditorGUILayout.BeginHorizontal();
                    for (int w = 0;
                         w < script.WordsCount && w < 20;
                         w++) //max X words for performance
                    {
                        EditorGUILayout.BeginVertical();
                        if (GUILayout.Button(script.Words[w].text,
                                EditorStyles.miniButton))
                            script.SetVisibilityWord(w,
                                !script.Characters[
                                        script.Words[w].firstCharacterIndex]
                                    .isVisible);

                        EditorGUILayout.BeginHorizontal();
                        for (int i = script.Words[w].firstCharacterIndex;
                             i <= script.Words[w].lastCharacterIndex;
                             i++)
        ...
        {
            const string helpConstantTags =
                "How many of these effects will be applied to the entire text";
            ...
            void DrawDefaultArray<T>(ref bool enabledOption, string name,
                SerializedProperty array,
                Database<T> database) where T : ScriptableObject, ITagProvider
            {
                int size = array.arraySize;
                EditorGUILayout.BeginHorizontal();
                enabledOption = EditorGUILayout.Foldout(enabledOption,
                ...
                bool IsTagValid(string tag)
                {
                    ...
                    return string.IsNullOrEmpty(tag) ||
                           database.ContainsKey(
                               tag.Split(' ')
                                   [0]); //splits in case of attributes
                }
                ...
                if (database)
                {
                    for (int i = 0; i < array.arraySize && areAllTagsValid; i++)
                        if (!IsTagValid(array.GetArrayElementAtIndex(i)
                                .stringValue))
                            areAllTagsValid = false;

                    if (!areAllTagsValid)
                        GUILayout.Box(TexturesLoader.WarningIcon,
                            EditorStyles.label, GUILayout.Width(20),
                            GUILayout.Height(20));
                }
                ...
                if (enabledOption)
                {
                    ...
                    GUI.enabled = false;
                    EditorGUILayout.LabelField(
                        defaultTagsMode.intValue ==
                        (int)TAnimCore.DefaultTagsMode.Fallback
                            ? helpFallbackTags
                            : helpConstantTags,
                        EditorStyles.wordWrappedMiniLabel);
                    GUI.enabled = true;
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        if (increasing && size > 1) //new element is empty
                            array.GetArrayElementAtIndex(size - 1).stringValue =
                                string.Empty;
                    ...
                    if (size > 0)
                    {
                        ...
                        for (int i = 0; i < array.arraySize; i++)
                        {
                            ...
                            EditorGUI.BeginChangeCheck();
                            tempString =
                                EditorGUILayout.DelayedTextField(temp,
                                    tempString);
                            if (EditorGUI.EndChangeCheck())
                            {
            ...
            if (useDefaultDatabases.boolValue)
            {
                ...
                if (settings)
                {
                    DrawDefaultArray(ref drawDefaultAppearancesTags,
                        "Appearances", defaultAppearancesTags,
                        settings.appearances.defaultDatabase);
                    DrawDefaultArray(ref drawDefaultBehaviorTags, "Behaviors",
                        defaultBehaviorsTags,
                        settings.behaviors.defaultDatabase);
                    DrawDefaultArray(ref drawDefaultDisappearancesTags,
                        "Disappearances", defaultDisappearancesTags,
                        settings.appearances.defaultDatabase);
                }
            }
            else
            {
                DrawDefaultArray(ref drawDefaultAppearancesTags, "Appearances",
                    defaultAppearancesTags, script.DatabaseAppearances);
                DrawDefaultArray(ref drawDefaultBehaviorTags, "Behaviors",
                    defaultBehaviorsTags, script.DatabaseBehaviors);
                DrawDefaultArray(ref drawDefaultDisappearancesTags,
                    "Disappearances", defaultDisappearancesTags,
        ...
        {
            EditorGUILayout.LabelField("Edit Effects & Actions",
                EditorStyles.boldLabel);
            EditorGUI.indentLevel++;
            ...
            EditorGUILayout.LabelField(
                useDefaultDatabases.boolValue
                    ...
                    : "Editing databases assigned only to this component.",
                EditorStyles.wordWrappedMiniLabel);
            GUI.enabled = true;
            EditorGUILayout.EndHorizontal();

            void DrawDatabaseField(ref bool foldoutToggle, string foldoutName,
                SerializedProperty field, DatabaseSharedDrawer drawer)
            {
                foldoutToggle =
                    EditorGUILayout.Foldout(foldoutToggle, foldoutName, true);
            ...
            if (useDefaultDatabases.boolValue)
            {
                ...
                else
                {
                    ...
                    SerializedProperty databaseBehaviorsField = settingsObject
                        .FindProperty(nameof(settings.behaviors))
                        .FindPropertyRelative(
                            nameof(settings.behaviors.defaultDatabase));
                    SerializedProperty databaseAppearancesField = settingsObject
                        .FindProperty(nameof(settings.appearances))
                        .FindPropertyRelative(
                            nameof(settings.appearances.defaultDatabase));
                    SerializedProperty databaseActionsField = settingsObject
                        .FindProperty(nameof(settings.actions))
                        .FindPropertyRelative(
                            nameof(settings.actions.defaultDatabase));
                    DrawDatabaseField(ref editAppearances,
                        "Appearances/Disappearances", databaseAppearancesField,
                        databaseAppearancesDrawer);
                    DrawDatabaseField(ref editBehaviors, "Behaviors",
                        databaseBehaviorsField, databaseBehaviorsDrawer);
                    DrawDatabaseField(ref editActions, "Actions",
                        databaseActionsField, databaseActionsDrawer);
                }
            }
            else
            {
