...
namespace Febucci.UI
{
    ...
    public class TypewriterByCharacter : TypewriterCore
    {
        ...
        protected override float GetWaitAppearanceTimeOf(int charIndex)
        {
            ...
            if (avoidMultiplePunctuationWait &&
                char.IsPunctuation(character)) //curr char is punctuation
                //next char is punctuation too, so skips this one
                if (charIndex < TextAnimator.CharactersCount - 1
                    && char.IsPunctuation(TextAnimator.Characters[charIndex + 1]
                        .info
                        .character))
            ...
            if (!waitForNewLines &&
                !TextAnimator.latestCharacterShown.info.isRendered)
            {
                bool
                    IsUnicodeNewLine(
                        ulong unicode) //Returns true if the unicode value represents a new line
                {
                    return unicode == 10 || unicode == 13;
                }

                //skips waiting for a new line
                if (IsUnicodeNewLine(Convert.ToUInt64(TextAnimator
                        .latestCharacterShown.info.character)))
                    return 0; //TODO test
            }
        ...
        protected override float GetWaitDisappearanceTimeOf(int charIndex)
        {
            return useTypewriterWaitForDisappearances
                ? GetWaitAppearanceTimeOf(charIndex) *
                  (1 / disappearanceSpeedMultiplier)
