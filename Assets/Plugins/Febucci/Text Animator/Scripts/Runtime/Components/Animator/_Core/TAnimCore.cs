...
namespace Febucci.UI.Core
{
    ...
    [HelpURL(
        "https://www.febucci.com/text-animator-unity/docs/how-to-add-effects-to-your-texts/")]
    public abstract class TAnimCore : MonoBehaviour
    {
        ...
        protected virtual void
            OnEnable() // things might have changed when disabled, e.g. autoSize etc.
        {
        ...
        public bool allLettersShown
        {
            get
            {
                ...
                if (firstVisibleCharacter == _maxVisibleCharacters)
                    return false;
                ...
                for (int i = 0; i < CharactersCount; i++)
                    if (characters[i].isVisible)
                    {
                        ...
                        if (characters[i].info.isRendered &&
                            characters[i].passedTime < characters[i].info
                                .appearancesMaxDuration)
                            return false;
                    }
                    else
                    {
        ...
        public bool anyLetterVisible
        {
            get
            {
                ...
                if (IsCharacterVisible(0) ||
                    IsCharacterVisible(CharactersCount - 1))
                    return true;
        ...
        /// </summary>
        [Tooltip(
            "Font size that will be used as reference to keep animations consistent/uniform at different scales.")]
        ...
        public AnimationsDatabase DatabaseBehaviors
        {
            get => useDefaultDatabases
                ? TextAnimatorSettings.Instance.behaviors.defaultDatabase
                : databaseBehaviors;
            set
            {
        ...
        [SerializeField] private StyleSheetScriptable styleSheet;

        public StyleSheetScriptable StyleSheet
        {
            get => useDefaultStyleSheet
        ...
        /// </remarks>
        public Vector2Int[] TypewriterDisabledRange { get; set; }
        ...
        {
            get => useDefaultDatabases
                ? TextAnimatorSettings.Instance.actions.defaultDatabase
                : databaseActions;
            set
            {
        ...
        private struct DefaultRegion
        {
            ...
            public DefaultRegion(string tagID, VisibilityMode visibilityMode,
                AnimationScriptableBase scriptable, string[] tagWords)
            {
        ...
        [SerializeField]
        private string[] defaultDisappearancesTags = { "fade" };
        ...
        private void ConvertText(string textToParse, ShowTextMode showTextMode)
        {
            ...
            void PopulateCharacters(bool resetVisibility)
            {
                ...
                for (int i = 0; i < CharactersCount; i++)
                {
                    ...
                    characters[i].info.disappearancesMaxDuration =
                        CalculateRegionMaxDuration(disappearances);
                    characters[i].info.appearancesMaxDuration =
                        CalculateRegionMaxDuration(appearances);
                }
            }
            ...
            void CalculateWords()
            {
                ...
                for (int i = 0; i < CharactersCount; i++)
                {
                    ...
                    if (tempLength > 0)
                    {
                        ...
                        currentFirstIndex +=
                            tempLength + 1; //removes additional space
                        wordIndex++;
                    }
                    else
                    {
                        currentFirstIndex
                            ++; //proceeds to shift white spaces etc.
                    }
            ...
            {
                for (int i = 0; i < CharactersCount; i++) HideCharacterTime(i);
            }
            ...
            bool IsCharacterInsideAnyEffect(int charIndex,
                AnimationRegion[] regions)
            {
                foreach (var region in regions)
                foreach (var range in region.ranges)
                    if (charIndex >= range.indexes.x &&
                        (range.indexes.y == int.MaxValue ||
                         charIndex < range.indexes.y))
                        return true;

                return false;
            }

            void AddFallbackEffectsFor<T>(ref AnimationRegion[] currentEffects,
                VisibilityMode visibilityMode, Database<T> database,
                string[] defaultEffectsTags) where T : AnimationScriptableBase
            {
                if (!database) return;

                if (defaultEffectsTags == null ||
                    defaultEffectsTags.Length == 0) return;
                ...
                foreach (var tag in defaultEffectsTags)
                {
                    if (string.IsNullOrEmpty(tag))
                    {
                        if (Application.isPlaying)
                            Debug.LogError(
                                $"Empty tag as default effect in database {database.name}. Skipping.",
                                gameObject);
                        continue;
                    }
                ...
                Array.Resize(ref currentEffects,
                    currentEffects.Length + defaultRegions.Count);
            ...
            if (!settings)
            {
                CharactersCount = 0;
                Debug.LogError(
                    "Text Animator Settings not found. Skipping setting the text to Text Animator.");
                return;
            }
            ...
            var ruleBehavior = new AnimationParser<AnimationScriptableBase>(
                settings.behaviors.openingSymbol, '/',
                settings.behaviors.closingSymbol, VisibilityMode.Persistent,
                databaseBehaviors);
            var ruleAppearance = new AnimationParser<AnimationScriptableBase>(
                settings.appearances.openingSymbol, '/',
                settings.appearances.closingSymbol, VisibilityMode.OnVisible,
                databaseAppearances);
            var ruleDisappearance =
                new AnimationParser<AnimationScriptableBase>(
                    settings.appearances.openingSymbol, '/', '#',
                    settings.appearances.closingSymbol, VisibilityMode.OnHiding,
                    databaseAppearances);
            ActionParser ruleActions = new(settings.actions.openingSymbol, '/',
                settings.actions.closingSymbol, databaseActions);
            EventParser ruleEvents = new('<', '/', '>');

            var typewriterDisabled =
                new PlainTagParser(settings.controlTags.disableTypewriter, '<',
                    '/', '>');
            ...
            textWithoutTextAnimTags =
                StyleSheet
                    ? TextParser.ParseText(_text,
                        new StylesParser('<', '/', '>', StyleSheet))
                    : _text;
            ...
            AddFallbackEffectsFor(ref behaviors, VisibilityMode.Persistent,
                databaseBehaviors, defaultBehaviorsTags);
            AddFallbackEffectsFor(ref appearances, VisibilityMode.OnVisible,
                databaseAppearances, defaultAppearancesTags);
            AddFallbackEffectsFor(ref disappearances, VisibilityMode.OnHiding,
                databaseAppearances, defaultDisappearancesTags);

            //Initializes only animations that are being used
            foreach (var behavior in behaviors)
                behavior.animation.InitializeOnce();
            foreach (var appearance in appearances)
                appearance.animation.InitializeOnce();
            foreach (var disappearance in disappearances)
                disappearance.animation.InitializeOnce();
            ...
            time.UpdateDeltaTime(timeScale == TimeScale.Unscaled
                ? Time.unscaledDeltaTime
                : Time.deltaTime);

            if (isResettingTimeOnNewText &&
                showTextMode != ShowTextMode.Refresh)
                time.RestartTime();
        }
        ...
        {
            ConvertText(text,
                hideText ? ShowTextMode.Hidden : ShowTextMode.Shown);
        }
        ...
        private void SetTypewriterText(string text)
        {
            ...
            if (string.IsNullOrEmpty(
                    text)) //forces clearing the mesh during the tempFix, without the <noparse> tags
            ...
            bool canPlayEffects = true)
        {
            ...
            else
            {
                ...
                if (characters[index].info.disappearancesMaxDuration >
                    characters[index].passedTime &&
                    characters[index].passedTime >= characters[index].info
                        .appearancesMaxDuration)
                    characters[index].passedTime = characters[index].info
            ...
            bool canPlayEffects = true)
        {
            ...
            for (int i = Mathf.Max(word.firstCharacterIndex, 0);
                 ...
                 i++) SetVisibilityChar(i, isVisible, canPlayEffects);
        }
        ...
        /// <param name="canPlayEffects"></param>
        public void SetVisibilityEntireText(bool isVisible,
            bool canPlayEffects = true)
        {
            for (int i = 0; i < CharactersCount; i++)
        ...
        private void Update()
        {
            ...
            if (HasChangedText(textWithoutTextAnimTags))
            {
                ...
                ConvertText(GetOriginalTextFromSource(),
                    ShowTextMode.UserTyping);
                return;
            }

            //--Animates in Core Loop--
            if (animationLoop == AnimationLoop.Update)
                Animate(timeScale == TimeScale.Unscaled
                    ? Time.unscaledDeltaTime
                    : Time.deltaTime);
        }
        ...
        {
            return i >= firstVisibleCharacter && i < _maxVisibleCharacters &&
        ...
        private void ProcessAnimationRegions(AnimationRegion[] regions)
        {
            ...
            foreach (var range in region.ranges)
            {
                region.SetupContextFor(this,
                    range.modifiers); //TODO index instead of passing modifier by value

                for (int i = range.indexes.x;
                     i < range.indexes.y && i < CharactersCount;
                     i++)
                {
                    if (characters[i].passedTime <= 0) continue;
                    if (!region.IsVisibilityPolicySatisfied(
                            IsCharacterAppearing(i))) continue;
        ...
        private void AnimateText()
        {
            ...
            for (int i = 0; i < CharactersCount && i < characters.Length; i++)
            {
                ...
                else
                {
                    ...
                    if (characters[i].passedTime <=
                        0) // "<=" to force hiding characters when TimeScale = 0 
                    {
                        characters[i].passedTime = 0;
                        characters[i].Hide();
                    }
        ...
        [Obsolete(
            "Please use TextAnimatorSettings.Instance.appearances.enabled instead")]
        public static bool effectsAppearancesEnabled =>
            TextAnimatorSettings.Instance.appearances.enabled;

        [Obsolete(
            "Please use TextAnimatorSettings.Instance.behaviors.enabled instead")]
        public static bool effectsBehaviorsEnabled =>
            TextAnimatorSettings.Instance.behaviors.enabled;
