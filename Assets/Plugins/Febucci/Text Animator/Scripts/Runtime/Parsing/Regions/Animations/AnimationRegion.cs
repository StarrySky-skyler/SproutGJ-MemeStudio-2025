...
namespace Febucci.UI.Core.Parsing
{
    ...
    public class AnimationRegion : RegionBase
    {
        ...
        public AnimationRegion(string tagId, VisibilityMode visibilityMode,
            AnimationScriptableBase animation) : base(tagId)
        {
        ...
        public virtual void SetupContextFor(TAnimCore animator,
        ...
        public void OpenNewRange(int startIndex, string[] tagWords)
        {
            ...
            TagRange range = new(new Vector2Int(startIndex, int.MaxValue));

            //Adds modifiers
            for (int i = 1;
                 i < tagWords.Length;
                 i++) //starts from 1 'cos skips tag name
            {
                string tag = tagWords[i];
                int equalIndex = tag.IndexOf('=');
                if (equalIndex <= 0) continue; //invalid modifier

                if (FormatUtils.TryGetFloat(tag.Substring(equalIndex + 1), 0,
                        out var result))
                {
                    Array.Resize(ref range.modifiers,
                        range.modifiers.Length + 1);
