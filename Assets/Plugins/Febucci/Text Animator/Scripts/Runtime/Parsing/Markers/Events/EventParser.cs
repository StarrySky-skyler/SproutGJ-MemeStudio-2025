...
namespace Febucci.UI.Core.Parsing
{
    public class EventParser : TagParserBase
    {
        ...
        public EventParser(char openingBracket, char closingBracket,
            char closingTagSymbol)
            ...
            StringBuilder finalTextBuilder, int internalOrder)
        {
            ...
            {
                string eventName =
                    textInsideBrackets.Substring(1, indexOfEquals - 1);
                string parameters =
                    textInsideBrackets.Substring(indexOfEquals + 1);

                //TODO fast strip
                textEvent = new EventMarker(eventName, realTextIndex,
                    internalOrder, parameters.Replace(" ", "").Split(','));
            }
            else
            {
                textEvent = new EventMarker(textInsideBrackets.Substring(1),
                    realTextIndex, internalOrder, new string[0]);
            }
