...
namespace Febucci.UI.Core.Parsing
{
    public sealed class ActionParser : TagParserBase
    {
        ...
        public ActionParser(char startSymbol, char closingSymbol,
            char endSymbol, ActionDatabase actionDatabase)
            ...
            StringBuilder finalTextBuilder, int internalOrder)
        {
            ...
            string actionName = equalIndex == -1
                ? textInsideBrackets
                : textInsideBrackets.Substring(0, equalIndex);
            actionName =
                actionName.ToLower(); //action names are case insensitive
            ...
            {
                string parameters =
                    textInsideBrackets.Substring(equalIndex + 1);
                textAction = new ActionMarker(actionName, realTextIndex,
                    internalOrder, parameters.Replace(" ", "").Split(','));
            }
            else
            {
                textAction = new ActionMarker(actionName, realTextIndex,
                    internalOrder, new string[0]);
            }
