...
namespace Febucci.UI.Core.Parsing
{
    ...
    public static class TextParser
    {
        public static string ParseText(string text,
            params TagParserBase[] rules)
        {
            if (rules == null || rules.Length == 0)
            {
                Debug.LogWarning(
                    "No rules were provided to parse the text. Skipping");
                return text;
            }
            ...
            */
            ...
            for (int textIndex = 0, realTextIndex = 0;
                 textIndex < len;
                 textIndex++)
            {
                foundTag = false;
                ...
                if (allowParsing && !foundTag)
                    foreach (var rule in rules) //tries rich tags
                        if (characters[textIndex] == rule.startSymbol)
                            for (int endIndex = textIndex + 1;
                                 endIndex < len && !foundTag;
                                 endIndex++)
                            {
                                ...
                                if (characters[endIndex] == rule.endSymbol)
                                {
                                            ...
                                            textIndex))
                                    {
                                        foundTag = true;
                                        textIndex =
                                            endIndex; //Tag processed, skips others
                                        break;
                                    }
                                }
                            }
