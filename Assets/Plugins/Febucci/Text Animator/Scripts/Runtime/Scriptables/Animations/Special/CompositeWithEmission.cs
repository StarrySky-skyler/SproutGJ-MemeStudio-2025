...
namespace Febucci.UI.Effects
{
    ...
    public sealed class CompositeWithEmission : AnimationScriptableBase
    {
        ...
        public override void ApplyEffectTo(ref CharacterData character,
            TAnimCore animator)
        {
            float timePassed = timeMode.GetTime(animator.time.timeSinceStart,
                character.passedTime, character.index);
            ...
            for (int i = 0; i < TextUtilities.verticesPerChar; i++)
            {
                ...
                character.current.colors[i] =
                    Color32.LerpUnclamped(prev.colors[i],
                        character.current.colors[i], weight);
            }
        }
