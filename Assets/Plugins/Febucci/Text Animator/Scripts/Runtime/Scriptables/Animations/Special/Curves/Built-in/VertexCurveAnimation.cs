...
namespace Febucci.UI.Effects
{
    ...
    public sealed class VertexCurveAnimation : AnimationScriptableBase
    {
        ...
        [SerializeField] private AnimationData[] animationPerVertexData =
            new AnimationData[TextUtilities.verticesPerChar];
            ...
            TAnimCore animator)
        {
            timePassed = timeMode.GetTime(
                animator.time.timeSinceStart * timeSpeed,
                character.passedTime * timeSpeed, character.index);
            if (timePassed < 0)
                return;
            ...
            {
                if (animationPerVertexData[i].TryCalculatingMatrix(character,
                        timePassed, weight, out matrix, out offset))
                    character.current.positions[i] =
                        matrix.MultiplyPoint3x4(character.current.positions[i] -
                                                offset) + offset;

                if (animationPerVertexData[i].TryCalculatingColor(character,
                        timePassed, weight, out color))
                    character.current.colors[i] = Color32.LerpUnclamped(
                        character.current.colors[i], color,
        ...
        private void ClampVertexDataArray()
        {
            ...
            {
                Debug.LogError(
                    "Vertex data array must have four vertices. Clamping/Resizing to four.");
