...
namespace Febucci.UI.Examples
{
    ...
    internal class ExampleEvents : MonoBehaviour
    {
        ...
        private void OnMessage(EventMarker eventData)
        {
            switch (eventData.name)
            {
                case "face":
                    ...
                    {
                        Debug.LogWarning(
                            $"You need to specify a sprite index! Dialogue: {dialogueIndex}");
                        return;
                    }
                    ...
                    if (TryGetInt(eventData.parameters[0], out int spriteIndex))
                    {
                        if (spriteIndex >= 0 && spriteIndex < faces.Length)
                            ...
                            Debug.Log(
                                $"Sprite index was out of range. Dialogue: {dialogueIndex}");
                    }
                ...
                case "crate":
                    ...
                    {
                        Debug.LogWarning(
                            $"You need to specify a crate index! Dialogue: {dialogueIndex}");
                        return;
                    }
        ...
        private IEnumerator AnimateCrate(int crateIndex)
        {
            ...
            Vector3 targetScale = new(initialScale.x * 1.2f,
                initialScale.y * .6f, initialScale.z);
            float t = 0;
            ...
            while (t <= duration)
            {
                ...
                crate.localScale =
                    Vector3.LerpUnclamped(initialScale, targetScale, pct);
                yield return null;
            }
